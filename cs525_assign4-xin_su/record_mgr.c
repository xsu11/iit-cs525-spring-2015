...
RC updateReservedPage(RM_TableData *rel, PageNumber reservedPageNum,
	...
	if (value == FALSE && pageNum <= firstFreePageNum) {
		...
		if(firstFreePageNum != (reservedPageNum + 32000)){
			bitOffset++;
		}
		...
		//printf("#############bitOffset = %d\n",bitOffset);
		//printf("#############maxBitOffset = %d\n",maxBitOffset);
		//printf("######################### = %d\n",getBit(page->data, bitOffset));
...
RC next(RM_ScanHandle *scan, Record *record) {
	...
	if (R_ID->slot == (pageMaxRecords - 1)) {
		if (R_ID->page == totalPages - 1) {
			// set RECORD back to the first record position
			R_ID->page = FIRST_RESERVED_PAGE + 1;
			R_ID->slot = -1;
	...
	*result = isFound;
	...
	while (curRecordPos < (totalRecords + 1) && R_ID->page < totalPages
		...
		if ((R_ID->page % dataPages) != FIRST_RESERVED_PAGE
			...
			while (curRecordPos < (totalRecords + 1)
				...
				if (isOccupied < 0) {
					...
					rc = -99;
					rc = getRecord(scan->rel, *R_ID, record);
					...
					rc = -99;
					rc = evalExpr(record, scan->rel->schema,
							(Expr *) (scan->mgmtData), result);
		 ...
		 * or: R_ID->slot = pageMaxRecords: we need to move to the next page
	...
	if ((*result)->v.boolV == TRUE) {
		rc = RC_OK;
	} else if (curRecordPos == totalRecords || R_ID->page == totalPages) {
		R_ID->page = record->id.page;
		R_ID->slot = record->id.slot;
