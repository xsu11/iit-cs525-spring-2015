...
/* BEGIN 2015-04-13, commented by Xin Su <xsu11@hawk.iit.edu>, move this type define to tables.h */
/*
 // dynamic string
 ...
 */
/* END 2015-04-13, commented by Xin Su <xsu11@hawk.iit.edu>, move this type define to tables.h */
...
char *serializeAttr(Record *record, Schema *schema, int attrNum) {
	...
	case DT_BOOL: {
		...
		APPEND(result, "%s:%s", schema->attrNames[attrNum],
				val ? "TRUE" : "FALSE");
	}
		break;
	default:
...
char *schemaToDataFlow(Schema *schema) {
	int i;
	VarString *result;
	MAKE_VARSTRING(result);
